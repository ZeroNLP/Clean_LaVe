...
def fine_tune_v3(args):
    ...
    sortByPlabelConfidence_en = pickle_load(args.sortByPlabelConfidence_path_en)
    train_data_en = pickle_load(args.train_path_en)
    ...
    print("prepare data from target language")
    data = prepare4mnil(sortByPlabelConfidence, train_data,  args.ratio, label2template)
    print("prepare data from english")
    data_en = prepare4mnil_count(sortByPlabelConfidence_en, train_data_en, int(args.factor * args.ratio * len(sortByPlabelConfidence)), label2template)
...
if __name__=="__main__":
    ...
    parser.add_argument('--cuda_index', type=int,default=0, help='as name')
    ...
    parser.add_argument("--sortByPlabelConfidence_path_en", type=str,default='/data/tywang/transfer/tac_configs/NLNL_tac_RT0.05_SD17.pkl', 
                        help="select数据的路径")
    parser.add_argument('--train_path_en',type=str,default="",
                        help = "as named")
    parser.add_argument('--factor', type=float,default=1, help='用0.05*Ntrain的数据finetune')
